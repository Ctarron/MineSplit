...
stopwatch create ctarron.splits:time
...
scoreboard players set .60 ctarron.splits.constant 60
scoreboard players set .1000 ctarron.splits.constant 1000
...
data merge storage ctarron:splits {aux: {name: "$(name)", index: 0, index_dialog: "$(index_dialog)", index_previous: -1, n: 0}, aux_time: {pb: [""], s: [""], split: "", ms: {color: "green", bold: 0b, text: ".00"}, h: ["   "], name: "", index: 0, time: 22988, m: ["0:"], n: 1}}
